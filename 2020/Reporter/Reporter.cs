...
public class Reporter : MonoBehaviour {
	...
	public void Initialize()
	{
		...
		{
			try{
				gameObject.SendMessage( "OnPreStart", SendMessageOptions.DontRequireReceiver );
			}
		...
		copyContent = new GUIContent("",getImage("software",size.x,size.y),"Copy log");
...
//		show 		= (PlayerPrefs.GetInt( "Reporter_show" )==1)?true:false;
    ...
    void OnDestroy()
    {
        Reporter[] objs = GameObject.FindObjectsOfType<Reporter>();
...
//		StartCoroutine("readInfo");
	...
	void drawToolBar(){
		...
		if( GUILayout.Button(closeContent ,barStyle, GUILayout.Width(size.x*2) , GUILayout.Height(size.y*2)))
		{
			...
			try{
				gameObject.SendMessage( "OnHideReporter", SendMessageOptions.DontRequireReceiver);
			}
			catch( System.Exception e ){
				Debug.LogException( e );
			}
		...
		{
			string log = selectedLog.condition + "\n\n" + selectedLog.stacktrace;

			//if(Application.isWebPlayer)
			//{
			//	TextEditor te = new TextEditor();
			//	te.content = new GUIContent(log);
			//	te.SelectAll();
			//	te.Copy();
			//}
			//else
			{
				PropertyInfo systemCopyBufferProperty = typeof(GUIUtility).GetProperty("systemCopyBuffer", BindingFlags.Static | BindingFlags.NonPublic);
				systemCopyBufferProperty.SetValue(null, log, null);
			}
		}
...
//			GUILayout.Space( size.y*0.25f );
	...
	public void OnGUIDraw()
	{
		...
		logsRect.height = Screen.height * div - size.y * 2f ;
		
		stackRectTopLeft.x = 0f ;
		stackRect.x = 0f ;
		stackRectTopLeft.y = Screen.height * div ;
		stackRect.y = Screen.height * div ;
		stackRect.width = Screen.width ;
		stackRect.height = Screen.height * (1 - div) - size.y ;
	...
	void doShow(){
		...
		try{
			gameObject.SendMessage( "OnShowReporter", SendMessageOptions.DontRequireReceiver);
		}
		catch(System.Exception e ){
			Debug.LogException( e );
		}
...
//		gcTotalMemory = (float)SystemInfo.systemMemorySize ;
	...
	void CaptureLog (string condition, string stacktrace, LogType type)
	{
		...
		if(condition.Length > 16000)
			condition = condition.Substring(0, 16000);
		...
		if( logsDic.ContainsKey(key))// && logsDic[key].stacktrace == stacktrace )
		{
			isNew = false ;
			logsDic[key].count ++;
		}
		...
		try
		{
			gameObject.SendMessage( "OnLog" ,log, SendMessageOptions.DontRequireReceiver );
		}
		catch( System.Exception e )
		{
			Debug.LogException( e );
